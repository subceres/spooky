...
void opcontrol() {
	...
	while (true) {
		...
		bool scoreLow=master.get_digital(E_CONTROLLER_DIGITAL_L2);
		bool chainUp=master.get_digital(E_CONTROLLER_DIGITAL_UP);
		bool chainDown=master.get_digital(E_CONTROLLER_DIGITAL_DOWN);
		...
		// Chain move code
		if (chainUp==true and chainDown==false){
			chainMotor.move(127);
		} else if (chainUp==false && chainDown==true){
			chainMotor.move(-127);
		} else {
			chainMotor.move(0);
		}
